...
- **AI video subtitles on YouTube.** ImmerseFree turns captions on by itself and draws its own translated line underneath. Three things happen before the model is called. **Semantic merging:** player cues are cut for display timing, so a sentence often arrives in pieces; they are merged back into whole sentences first (gap over 400 ms starts a new sentence; a group is capped at 7 seconds and 160 characters; an abbreviation list keeps `Mr.` and `e.g.` from splitting one). **Three-layer context:** each batch of 12 sentences carries the video's title, channel and description, the glossary terms that actually occur in that batch, and the last 8 completed sentences as bilingual pairs — plus a short-sentence flag, at no extra API call, for cues on screen under 1.2 s or needing more than 20 characters per second to read. **Line breaking in the browser:** the translation stays on one line; if it overflows, the font shrinks to a floor of 65%, and only then does it wrap to two lines at the original size. The model is never asked to insert line breaks, because it cannot know the player width and a stray newline inside the JSON response can lose an entire batch.
- **YouTube AI 字幕。** ImmerseFree 會自己把字幕打開，再在下方畫出自己的譯文行。呼叫模型之前會做三件事。**語意合併：** 播放器的 cue 是照顯示節奏切的，一句話常被切成好幾片，會先合併回完整句子（間隔超過 400 毫秒算新句；一組上限 7 秒與 160 字元；另有縮寫表避免 `Mr.`、`e.g.` 把句子切斷）。**三層上下文：** 每批 12 句會帶上影片的標題、頻道與描述、這批裡真的出現的術語，以及最近 8 句已完成句子的雙語對；另外搭載極短句標記，不多花一次 API 呼叫，處理停留不到 1.2 秒或要用每秒 20 字元以上才讀得完的句子。**斷行在瀏覽器裡算：** 譯文維持一行，放不下先把字級縮到最低 65%，縮到底才折成兩行且字級回到原大小。不會叫模型自己斷行，因為它不可能知道播放器多寬，而 JSON 回應裡跑出一個換行可能讓整批字幕消失。
- **Dual subtitles on Netflix and Disney+.** A streaming manifest lists every subtitle language at once and the player mounts one at a time; ImmerseFree fetches the track you did not pick and draws it under the one you did. No model, no quota, no cost, no waiting — and the second line is the distributor's own professional translation, timed by the same official file so the two lines stay aligned. It tries the player's mounted `<video>` text tracks, then the intercepted HLS/DASH playlist, then the subtitle URL the player already requested. Disney+ draws subtitles inside shadow DOM, so the shadow trees are walked too. If the subtitle you selected is already in your target language, the second line falls back to your fallback language, decided from the text actually on screen rather than the player's settings.
- **Netflix 與 Disney+ 雙軌字幕。** 串流平台的播放清單裡本來就同時列著所有語言的字幕，播放器只是一次掛一條；ImmerseFree 把你沒選到的那條抓下來，畫在你選的那條底下。不用模型、不吃額度、不花錢、不用等——而且第二行是片商自己的專業翻譯，時間碼出自同一份官方檔案，兩行不會跑掉。取得路徑依序是：播放器已掛在 `<video>` 上的字幕軌、攔截到的 HLS／DASH 播放清單、播放器自己請求過的字幕檔網址。Disney+ 把字幕畫在 shadow DOM 裡，所以連 shadow 樹也一起走。如果你選的字幕本來就是目標語言，第二行會改用備援語言，判斷依據是畫面上實際顯示的文字，不是播放器的設定值。
...
- **12 bilingual display themes:** `classic` (left border, the default), `underline`, `dashed`, `wavy`, `highlight`, `quote`, `faded`, `italic`, `bold`, `card`, `divider`, and `plain`. The options page previews all twelve at once using the same `content.css` that styles real pages, and switching a theme restyles pages you have already translated without retranslating anything. Only the three themes with a colour of their own — `highlight`, `quote`, `card` — need dark-mode overrides; the other nine derive their decoration from `currentColor` and follow the page's text colour automatically. The theme is a global setting, not a per-site one.
- **12 種雙語顯示主題：** `classic`（經典邊線，預設）、`underline` 底線、`dashed` 虛線、`wavy` 波浪線、`highlight` 高亮、`quote` 引用塊、`faded` 弱化、`italic` 斜體、`bold` 粗體、`card` 紙片、`divider` 分隔線、`plain` 無裝飾。選項頁會用實際套在網頁上的同一份 `content.css` 一次預覽十二種；換主題會直接替已翻好的頁面換裝，不需要重翻。只有三種自帶顏色的主題——`highlight`、`quote`、`card`——需要深色模式覆寫，另外九種的裝飾色由 `currentColor` 混出，會自動跟著頁面文字色走。主題是全域設定，不是按網站分別設定。
