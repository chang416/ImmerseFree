...
> [!IMPORTANT]
> If you used Safari before 0.8.0 and subtitles never left “Preparing”, rerun `Install or update Safari.command`. Releases before 0.8.0 did not grant the Safari extension outgoing network access, so its requests were silently dropped by the sandbox. See [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md).

> [!IMPORTANT]
> 如果你在 0.8.0 之前用過 Safari 版，而字幕永遠停在「準備中」，請重新執行 `Install or update Safari.command`。0.8.0 之前的版本沒有給 Safari 擴充功能對外連線的權限，它發出的請求會被沙盒安靜地丟掉。詳見 [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md)。
...
![A website translated by ImmerseFree, showing the original English paragraphs with Traditional Chinese translations below them](assets/translation-preview.png)
...
Full details, including which file defines each number above, are in [`docs/FEATURES.md`](FEATURES.md).

完整說明（包含上面每個數字是由哪個檔案定義的）在 [`docs/FEATURES.md`](FEATURES.md)。

If something does not work, start with [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md).

如果有東西不能用，先看 [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md)。
...
A streaming manifest lists **every** subtitle language at once. The player just mounts one of them at a time. ImmerseFree goes and fetches the one you did not pick, and draws it under the one you did. That means: no model call, no API key needed, no model quota consumed; subtitle files still need to be fetched — and the second line is the distributor's own professional translation, not a machine's guess at it. The original track timing is preserved. Different track segmentation or player timelines can still require alignment, and the extension uses the visible captions to calibrate it.

串流平台的播放清單裡本來就同時列著**所有**語言的字幕，播放器只是一次掛一條。ImmerseFree 把你沒選到的那一條抓下來，疊在你選的那條底下。所以：不呼叫模型、不需要 API key、不吃模型額度；仍需取得字幕檔——而且第二行是片商自己的專業翻譯，不是機器猜出來的。保留字幕原有時間碼；不同語言的分段或播放器時間基準仍可能不同，因此會比對畫面字幕進行校準。
...
在 Netflix 與 Disney+ 上，開啟 **雙軌字幕**，或按 `Alt + Shift + D`。這兩個平台本來就附了多條字幕軌，ImmerseFree 會把你沒選到的那條抓下來，疊在你選的那條底下——不用模型、不吃模型額度；仍需取得字幕檔，而且第二行是片商自己的翻譯。上面的 **Netflix 與 Disney+** 那一節有說明它是怎麼找到那條軌的。
...
- [`docs/FEATURES.md`](FEATURES.md) — bilingual feature overview; every number cites the file that defines it.
- [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md) — the problems reported most often and how to tell them apart.
- [`docs/OFFICIAL-SOURCES.md`](OFFICIAL-SOURCES.md) — evidence notes behind the installation and publishing instructions.

- [`docs/FEATURES.md`](FEATURES.md)——中英雙語功能總覽，每個數字都註明是哪個檔案定義的。
- [`docs/TROUBLESHOOTING.md`](TROUBLESHOOTING.md)——最常被回報的問題，以及怎麼分辨你遇到的是哪一個。
- [`docs/OFFICIAL-SOURCES.md`](OFFICIAL-SOURCES.md)——安裝與上架說明背後的查證紀錄。
